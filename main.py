...
def main():
	...
	"""global win, score2, location2, usernamemode, username, score_list, json_file, speedLimit,
	movingLeftSpeed, screenSize, screen, height, width, FPS, life, score, count, press, startAgain,
	flags, countdown, BLACK, WHITE, bg1, bg2, bg3, bg4, location, mode1, mode2, instruction, history,
	playmode, singlePlayer, duelPlayer, daytime, nighttime, clock, SCREENWIDTH, SCREENHEIGHT, BASEY,
	IMAGES, SOUNDS"""
...
def button(x2, y2, width, height, active_color, action = None): 
    ...
    if x2 + width > cur[0] > x2 and y2 + height > cur[1] > y2: 
        ...
        s.fill((68,219,52,128))                                # notice the alpha value in the color
        screen.blit(s, (x2,y2)) 
    else: 
        pass
...
def play_mode(score, life):
    ...
    if nighttime:
       IMAGES['pipe'] = (
            pygame.transform.rotate(pygame.image.load('pipe2.png').convert_alpha(), 180),
     ...
     while True:
       ...
       if duelPlayer:
            ...
            if live1 == False and live2 == False:
              if win == 1:
                my_font = pygame.font.Font("ComicSansMS3.ttf",30)
                text_surface = my_font.render("Player 1 wins", True, WHITE)
                rect = text_surface.get_rect(center = (170, 110))
                screen.blit(text_surface, rect)
              if win == 2:
                my_font = pygame.font.Font("ComicSansMS3.ttf",30)
                text_surface = my_font.render("Player 2 wins", True, WHITE)
                rect = text_surface.get_rect(center = (170, 110))
                screen.blit(text_surface, rect)
              pygame.display.flip()
              time.sleep(4)
